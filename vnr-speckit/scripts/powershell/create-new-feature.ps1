...
if ($Help) {
    Write-Host "Usage: ./.vnr-speckit/scripts/powershell/create-new-feature.ps1 [-Json] [-AllowExistingBranch] [-ShortName <name>] [-Number N] [-Timestamp] <feature description>"
    Write-Host ""
    Write-Host "Options:"
    ...
    Write-Host ""
    Write-Host "Examples:"
    Write-Host "  ./.vnr-speckit/scripts/powershell/create-new-feature.ps1 'Add user authentication system' -ShortName 'user-auth'"
    Write-Host "  ./.vnr-speckit/scripts/powershell/create-new-feature.ps1 'Implement OAuth2 integration for API'"
    Write-Host "  ./.vnr-speckit/scripts/powershell/create-new-feature.ps1 -Timestamp -ShortName 'user-auth' 'Add user authentication'"
    exit 0
}

# Check if feature description provided
if (-not $FeatureDescription -or $FeatureDescription.Count -eq 0) {
    Write-Error "Usage: ./.vnr-speckit/scripts/powershell/create-new-feature.ps1 [-Json] [-AllowExistingBranch] [-ShortName <name>] [-Number N] [-Timestamp] <feature description>"
    exit 1
}
...
# Use common.ps1 functions which should prioritize .vnr-speckit over git
...
if (-not (Test-Path -PathType Leaf $specFile)) {
    ...
    if (-not $template) {
        $template = Join-Path $repoRoot '.vnr-speckit/templates/spec-template.md'
    }
