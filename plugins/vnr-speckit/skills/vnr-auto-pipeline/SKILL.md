...
- **Song song**: Steps 2a+2b dispatch cả 2 Agent tool calls trong cùng 1 message. Steps 5+6 tương tự.
- **Constitution**: `vnr-speckit/memory/constitution.md` là tài liệu quy tắc gốc — mọi agent phải tuân thủ.
- **Wiki context**: Các bước Plan (1), QC (2), Implement (3), Report (9) phải đọc `docs/wiki/` trước khi thực hiện. Xem hướng dẫn tại `vnr-speckit/skills/vnr-wiki/SKILL.md`.
- **Dual-repo git**: Khi tạo feature branch hoặc commit, phải thực hiện **riêng** cho từng repo (`cd src/backend` và `cd src/frontend`).
...
| 1a | vnr-planner | `vnr-speckit/agents/vnr-planner.md` | Skill tool: `vnr-plan` |
| 1b | vnr-task-breaker | `vnr-speckit/agents/vnr-task-breaker.md` | Skill tool: `vnr-tasks` |
| 2a | vnr-qc-generator | `vnr-speckit/agents/vnr-qc-generator.md` | Agent tool (song song với 2b) |
| 2b | vnr-testcase-writer | `vnr-speckit/agents/vnr-testcase-writer.md` | Agent tool (song song với 2a) |
| 3 | vnr-developer | `vnr-speckit/agents/vnr-developer.md` | Skill tool: `vnr-implement` |
| 4 | vnr-test-engineer | `vnr-speckit/agents/vnr-test-engineer.md` | Agent tool |
| 5 | vnr-arch-reviewer | `vnr-speckit/agents/vnr-arch-reviewer.md` | Agent tool (song song với 6) |
| 6 | vnr-sec-reviewer | `vnr-speckit/agents/vnr-sec-reviewer.md` | Agent tool (song song với 5) |
| 7 | — | — | CLI: `cd src/backend && dotnet test` / `cd src/frontend && npm test` |
| 8 | — | — | CLI: `cd src/frontend && npx playwright test` |
| 9 | vnr-tech-writer | `vnr-speckit/agents/vnr-tech-writer.md` | Agent tool |
...
Skill `vnr.plan` sẽ đọc `vnr-speckit/agents/vnr-planner.md` nội bộ, thực hiện:
...
Skill `vnr.tasks` sẽ đọc `vnr-speckit/agents/vnr-task-breaker.md` nội bộ, sinh `tasks.md`.
...
Dùng Agent tool:
  ...
  prompt: |
    Đọc và tuân theo system prompt: vnr-speckit/agents/vnr-qc-generator.md
...
Skill `vnr.implement` sẽ đọc `vnr-speckit/agents/vnr-developer.md` nội bộ, thực hiện:
...
Dùng Agent tool:
  subagent_type: "general-purpose"
  prompt: |
    Đọc và tuân theo system prompt: vnr-speckit/agents/vnr-test-engineer.md
...
Dùng Agent tool:
  subagent_type: "general-purpose"
  prompt: |
    Đọc và tuân theo system prompt: vnr-speckit/agents/vnr-arch-reviewer.md

    FEATURE: <feature>

    ĐỌC BẮT BUỘC:
    - vnr-speckit/standards/backend/02-architecture-and-structure.md
    - vnr-speckit/standards/frontend/02-architecture-and-structure.md
    - specs/<feature>/contracts/api-commitments.md (nếu có)
    - docs/raw/api-http-contracts.md (nếu có)
    - vnr-speckit/memory/constitution.md (Principle II)
...
Dùng Agent tool:
  subagent_type: "general-purpose"
  prompt: |
    Đọc và tuân theo system prompt: vnr-speckit/agents/vnr-sec-reviewer.md
    ...
    - vnr-speckit/standards/backend/03-permission.md
    - vnr-speckit/standards/frontend/03-permission.md
    - vnr-speckit/hooks/security-hooks.json (scan patterns)
    - vnr-speckit/memory/constitution.md (Principle III)
...
Dùng Agent tool:
  subagent_type: "general-purpose"
  prompt: |
    Đọc và tuân theo system prompt: vnr-speckit/agents/vnr-tech-writer.md
