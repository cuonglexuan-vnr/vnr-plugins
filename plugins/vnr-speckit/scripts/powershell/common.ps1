...
# Find repository root by searching upward for .vnr-speckit directory
...
function Find-VnrRoot {
    ...
    while ($true) {
        if (Test-Path -LiteralPath (Join-Path $current ".vnr-speckit") -PathType Container) {
            return $current
        }
...
# Get repository root, prioritizing .vnr-speckit directory over git
# This prevents using a parent git repo when Vnr plugin is initialized in a subdirectory
function Get-RepoRoot {
    # First, look for .vnr-speckit directory (Vnr's own marker)
    $vnrRoot = Find-VnrRoot
    if ($vnrRoot) {
        return $vnrRoot
    }

    # Fallback to git if no .vnr-speckit found
    try {
...
#   1. .vnr-speckit/templates/overrides/
#   2. .vnr-speckit/presets/<preset-id>/templates/ (sorted by priority from .registry)
#   3. .vnr-speckit/extensions/<ext-id>/templates/
#   4. .vnr-speckit/templates/ (core)
function Resolve-Template {
    ...
    $base = Join-Path $RepoRoot '.vnr-speckit/templates'
    ...
    $presetsDir = Join-Path $RepoRoot '.vnr-speckit/presets'
    ...
    $extDir = Join-Path $RepoRoot '.vnr-speckit/extensions'
