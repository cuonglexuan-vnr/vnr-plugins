...
<agent_to_use>Sử dụng vnr-mobile-developer agent — đọc `vnr-plugin/agents/vnr-mobile-developer.md` để hiểu vai trò, quy tắc Flutter/GetX/VnR widget và convention bắt buộc trước khi implement.</agent_to_use>
...
```powershell
vnr-plugin/scripts/powershell/check-prerequisites.ps1 -Json -RequireTasks -IncludeTasks
```
