...
- [ ] I18N: key mới có trong cả 2 ngôn ngữ (VN + EN).
- [ ] Permission directive dùng đúng syntax từ wiki convention.
- [ ] Route có auth guard + screen permission (theo wiki convention).
- [ ] Providers (API service, Facade, State) khai báo tập trung theo wiki `convention` entry.
- [ ] API service theo convention từ wiki (extend base class, inject config token).
- [ ] Không gọi API trực tiếp từ Component — phải qua Facade.
- [ ] Observable/stream cleanup: theo wiki `convention` entry (e.g., takeUntil pattern hoặc equivalent).
- [ ] Factory/builder labels là i18n key — không có raw string.
