...
| `vnr-plugin/standards/01-tech-stack.md`                          | Stack kỹ thuật (BE + FE) — .NET Framework 4.6.2, Angular 15 Micro-frontend, vnr-module design system                                           |
| `vnr-plugin/standards/02-architecture-and-structure.md`          | Architecture & source structure (BE layers, FE micro-frontend, Module Federation, vnr-module priority)                                          |
| `vnr-plugin/standards/03-data-and-auth.md`                       | Data permission (SP `Get_Data_Permission_New`), function permission (bitwise PrivilegeType), AuthGuard, `*vnrPermission` directive              |
| `vnr-plugin/standards/04-internal-be-framework-and-flow.md`      | BE internal framework — Controller hierarchy, ActionService, UnitOfWork, SP calling patterns, write flow                                        |
| `vnr-plugin/standards/05-internal-fe-framework-and-flow.md`      | **⚠️ BẮT BUỘC** — FE internal framework: VnrGrid, vnr-module Factory+Builder, Container/Presentational, Facade, API Service, vnr-module components (KHÔNG dùng `nz-*` trực tiếp) |
| `vnr-plugin/standards/06-team-principles-and-conventions.md`     | I18N (BE/FE), enums/constants, DB migration rules, reflection caveats, UI component priority                                                     |
