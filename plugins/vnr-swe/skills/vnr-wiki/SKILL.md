...
2. **Catalog theo folder** — bullet list `* [Title](folder/slug.md) — `type` — description` dưới mỗi heading `## <folder>/` (type inline để tìm nhanh entries theo loại: architecture, recipe, entity, workflow, …)
...
3. Cross-references là relative markdown links `[Title](folder/slug.md)` — KHÔNG dùng `[[id]]` wikilink (published bundle theo OKF). Theo link trực tiếp để mở page.
4. Mỗi page có frontmatter `description` (≤120 chars) — đó là nội dung xuất hiện trong index
