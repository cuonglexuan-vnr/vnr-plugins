...
compatibility: "Requires the vnr-swe plugin with docs/raw/_schema/ (README.md · the contract, frontmatter-spec.md, CONVENTIONS.md)"
metadata:
  author: "VNR"
user-invocable: true
---
...
3. **Raw frontmatter ≠ wiki frontmatter.** Skill này emit theo `_schema/frontmatter-spec.md` (`title, type, module, created, timestamp` + extras cho workflow). KHÔNG dùng format 8-field của wiki page (đó là việc của `vnr-wiki-sync`).
...
```
0a0. Đọc docs/raw/_schema/README.md          → CONTRACT tổng (OKF profile, 2-layer model, lời hứa "author chỉ cần frontmatter+type", bản đồ ERROR/WARN). Đây là nguồn chân lý — đọc TRƯỚC.
...
> Contract authority: `docs/raw/_schema/README.md` (the contract — start here); `OKF-SPEC.md` là chuẩn OKF gốc (vendored). Author chỉ cần **frontmatter hợp lệ + `type` không rỗng**; mọi thứ strict là việc của `vnr-wiki-sync`.
...
2b. DOMINANT type — chọn 1 từ 20 type hợp lệ trong frontmatter-spec.md (đã đọc ở Step 0). KHÔNG giữ bản sao ở đây để tránh lệch.
...
4a. Lấy required fields cho `type` đã chọn (từ frontmatter-spec).
    - Mọi file:            title, type, module, created, timestamp
    - type: workflow thêm: id (module.workflow.<name>), priority, status
4b. Điền field suy ra CHẮC CHẮN từ nội dung (vd: title, module nếu rõ; id CHỈ với type:workflow = module.workflow.<ten>).
    - created = ngày hôm nay (YYYY-MM-DD); timestamp = ngày hôm nay ISO 8601 (vd 2026-06-16T00:00:00Z). Lấy từ currentDate; nếu không có → để TODO, không bịa.
...
5e. Thêm provenance ở cuối body dưới heading "# Citations" (OKF §8):
      # Citations

      [1] Intake từ `<tên dump/_inbox file hoặc "paste">` bởi vnr-wiki-add — <ngày>.
```
...
6d. created đúng YYYY-MM-DD; timestamp đúng ISO 8601 (YYYY-MM-DDThh:mm:ssZ)?
...
### Step 9 — Validate + offer sync (one-command happy path)

```
9a. Validate file vừa tạo (authoring profile):
    node "<PLUGIN_DIR>/scripts/okf-validate.mjs" --root <project-root> --layer raw --path <raw-file>
    → phải 0 ERROR (frontmatter parseable + type non-empty). Fix nếu fail.
9b. Happy path 1 lệnh: hỏi "Đã tạo raw file. Chạy /vnr-wiki-sync <file> ngay?" [có → scoped sync đúng file vừa tạo | để sau]
9c. Nếu xử lý từ _inbox (batch queue tuỳ chọn) & user OK → đề nghị chuyển file nguồn sang
...
created: <YYYY-MM-DD>
timestamp: <YYYY-MM-DDThh:mm:ssZ>   # ISO 8601 (OKF) — lần thay đổi gần nhất
...
✅ vnr-wiki-add complete
  ...
  Bước tiếp: validate (okf-validate --layer raw) → /vnr-wiki-sync <file> (scoped)  (chưa tự chạy)
```
