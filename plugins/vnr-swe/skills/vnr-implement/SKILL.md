...
1. Đọc `docs/wiki/index.md` → tìm page dưới heading `## patterns/` (architecture/layout) và `## guides/` (recipe). Index catalog là OKF §6 bullet list theo folder — điều hướng theo folder heading, không theo type column.
...
1. **Setup** (optional): If `${CLAUDE_PLUGIN_ROOT}/scripts/powershell/check-prerequisites.ps1` exists, run it with `-Json -RequireTasks -IncludeTasks` from repo root and parse PLUGIN_DIR, FEATURE_DIR and AVAILABLE_DOCS list. If the script is absent, use `PLUGIN_DIR` = `${CLAUDE_PLUGIN_ROOT}`, and FEATURE_DIR as `specs/<feature>/`.
...
6. **Execute implementation — Sequential per-scope dispatch**:
   ...
   - Sau khi hoàn thành tất cả BE tasks: **chạy build** (command từ `docs/wiki/index.md` → page dưới `## guides/` (vd `how-to-build-and-test`) hoặc từ `plan.md` Technical Context → Commands)
   ...
   - Phase-by-phase theo thứ tự trong tasks.md
   - Parallel tasks `[P]` trong cùng phase: thực hiện song song
   - Sau khi hoàn thành tất cả FE tasks: **chạy build** (command từ `docs/wiki/index.md` → page dưới `## guides/` (vd `how-to-build-and-test`) hoặc từ `plan.md` Technical Context → Commands)
