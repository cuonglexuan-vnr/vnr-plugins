...
compatibility: "Requires the vnr-swe plugin installed in the project"
metadata:
  author: "vnr-plugins"
...
```
PLUGIN_DIR          = ${CLAUDE_PLUGIN_ROOT}  (the installed plugin root)
...
4. Transform the content:
   ...
   - **Add** a `customization` field inside the frontmatter to track the override origin:
     ...
     customization:
       source: "${CLAUDE_PLUGIN_ROOT}/skills/{name}/SKILL.md"
       type: "skill-override"
       created: "{YYYY-MM-DD}"
     ---
     ```
...
3. Transform the content:
   ...
   - **Add** a `customization` field inside the frontmatter to track the override origin:
     ...
     # ... (keep all original frontmatter fields)
     customization:
       source: "${CLAUDE_PLUGIN_ROOT}/agents/{name}.md"
       type: "agent-override"
       created: "{YYYY-MM-DD}"
     ---
     ```
...
4. Report:
   ...
   (${CLAUDE_PLUGIN_ROOT}/agents/{name}.md). You need to also override the skills that use
...
Scan ALL files in the plugin directory (`${CLAUDE_PLUGIN_ROOT}`, i.e. skills, agents, scripts) for references to the customized item. Only scan the plugin directory — do not scan user project files.
...
⚠️  {N} references found across {M} files in the plugin

┌─────────────────────────────────────────────────
│ DIRECT_CALL references
├─────────────────────────────────────────────────
│ 📄 ${CLAUDE_PLUGIN_ROOT}/skills/vnr-auto-pipeline/SKILL.md
...
│
│ 📄 ${CLAUDE_PLUGIN_ROOT}/skills/vnr-implement/SKILL.md
...
├─────────────────────────────────────────────────
│ 📄 ${CLAUDE_PLUGIN_ROOT}/scripts/powershell/check-prerequisites.ps1
...
Skills to update:
- ${CLAUDE_PLUGIN_ROOT}/skills/{skill1}/SKILL.md → override to .claude/skills/{skill1}/SKILL.md
- ${CLAUDE_PLUGIN_ROOT}/skills/{skill2}/SKILL.md → override to .claude/skills/{skill2}/SKILL.md
```
...
These rules **MUST** be enforced during customization. They serve as guardrails to maintain compatibility with the vnr-swe ecosystem.
